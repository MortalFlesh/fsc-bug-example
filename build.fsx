...
let project = "MF.FSCS.BugExample"
let summary = "Console application for a bug demonstration."
...
Target.create "AssemblyInfo" (fun _ ->
    let getAssemblyInfoAttributes projectName =
        ...
        let release =
            changeLog
            |> Option.map (fun changeLog -> ReleaseNotes.parse (System.IO.File.ReadAllLines changeLog |> Seq.filter ((<>) "## Unreleased")))
...
Target.create "Release" (fun _ ->
    ...
    ProjectSources.release
    |> Seq.collect (fun project -> runtimeIds |> List.map (fun runtimeId -> project, runtimeId))
